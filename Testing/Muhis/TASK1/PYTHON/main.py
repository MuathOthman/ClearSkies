...
yhteys = mysql.connector.connect(
    ...
    database='lentopeli',
    user='root',
    password='muhisonboss',
    autocommit=True
)

def airports(location): # This function asks user for a airport and tells him if the airport is active for airport serch.
...
def airports_old_user(): # This function asks user for a airport and tells him if the airport is active for old user.
    testi = []
    sql = "select ident from airport where scheduled_service =  'yes';"
    # print(sql)
    kursori = yhteys.cursor()
...
def icao2(): # This function asks the player another airport Icao-code.
    location = input("Syötä käytössä olevan lentoaseman ICAO-koodi: ").upper()
    return location

def same_airport(): # This function doesn't let you travel to the same airport you already are in.
    ...
    while True:
        ...
        if (location, ) in tulos:
            ...
            break
    return location
def newuser(nimi, location): #This function lets a player make a new user.
    ...
    if (nimi,) in tulos:
        ...
        ohjeet_newuser(nimi)
    return
def id(): # This function records ur user and adds it to the database.
    testi = []
    ...
    if kursori.rowcount > 0:
        ...
        for i in testi:
            # print(i)
            return i
def budjetti(nimi): #This function makes a random CO2-Budget to a player.
    import random
    ...
    tulos = kursori.fetchall()
    return
def ohjeet_newuser(nimi): # This function gives guides to a new player.
    ...
    if kursori.rowcount > 0:
        ...
        while True:
            ...
            if valinta == "1":
                ...
                print("Valitse 1-2!")
def kenttahaku(): # This function allows players to search for airports by giving country names.
...
def co2_consumed(nimi): # This collects how much you have consumed CO2
    while True:
        lista = []
...
def olduser(nimi): #this function lets you start where you let off if u have already signed in.
    sql = "SELECT screen_name FROM game"
    # print(sql)
    kursori = yhteys.cursor()
    kursori.execute(sql)
    ...
    return
def ohjeet_old_user(nimi): #If u are an olduser u can get guides by this function.
    sql = "select co2_budget from game "
    sql += "Where screen_name = '" + nimi + "'"
    kursori = yhteys.cursor()
    kursori.execute(sql)
...
def airports_old_user_restart(): #This lets an old user to restart the game.
    testi = []
    sql = "select ident from airport where scheduled_service =  'yes';"
    # print(sql)
    kursori = yhteys.cursor()
...
def restart(nimi): # This function clears ur old data at the database when you restart the game.
    ...
    #olduser(nimi)
    return
def pelikortti(): #If u want to know how you are doing in the game this function helps you by giving you a gametable.
    ...
    if kursori.rowcount > 0:
        for rivi in tulos:
            print(f"--------------------\nPelikorttisi:\nKulutettu CO2: {rivi[0]}\nCO2 budjetti: {rivi[1]}"
                  f"\nSijaintisi: {rivi[2]}\n--------------------")
def saa_reached(nimi): # This function collects the weathers u have collected.
    while True:
        lista = []
        ...
        if kursori.rowcount > 0:
            for i in tulos1:
                lista.append(i)
        return i[0]
def mainask(nimi): #this function adds ur game statistics to the database.
    testi = []
    i = co2_consumed(nimi)
    s = saa_reached(nimi)
    sql = "select co2_budget from game "
...
def leaderborad(): #This function makes a leaderboard where players can see how they results.
    testi = []
    ...
    return
def tulokset(nimi): #This function lets you know if u won or lost the game.
    i = co2_consumed(nimi)
    s = saa_reached(nimi)
    sql = "select count(*), co2_consumed from game, goal_reached where id = game_id"
    sql += " and screen_name= '" + nimi + "'"
...
def luuseri(nimi): # This function lets you know that u lost.
...
def win(nimi): # This function lets you know that you won the game.
...
def valikko_win(): # This function takes you back to the start if u win the game.
    print("--------------------")
    print("1, Aloita peli alusta")
    print("2, Lopeta peli")
    valinta = input("Valitse numero: ")
...
def valikko_loser(): # This function takes you back to the start if u lose the game.
    print("--------------------")
    print("1, Aloita peli alusta")
    print("2, Lopeta peli")
    valinta = input("Valitse numero: ")
...
def saa_budjetti(nimi): # With this one u can see how much you have ur budget left.
    testi = []
    testi = []
    ...
    print("--------------------")
    return
def tallenna(): # this function lets you store ur game and continue later.
    while True:
        print("--------------------")
...
def co2_lisaaminen(nimi): #This function adds the CO2-budget to you.
    ...
    tulos = kursori.fetchall()
    return
def laskuri(nimi): #This function counts how much you lose ur CO2-budget when you travel.
...
def id_find(nimi): #This function lets you find ur old user.
    testi = []
    sql = "select id from game "
    sql += "Where screen_name = '" + nimi + "'"
    #print(sql)
    ...
    return str(rivi[0])
def saa(nimi): #This function gives you the weather and records the weathers u have collected
    import random
